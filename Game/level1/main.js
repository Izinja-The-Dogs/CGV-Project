...
var healthsphere, lastHealthPickup = 0;
...
function init() {
	...
	scene = new t.Scene();
	scene.fog = new t.FogExp2(0xD6F1FF, 0.0015);
...
function render() {
	...
	healthsphere.rotation.x += 0.004
	healthsphere.rotation.y += 0.008;

	if (Date.now() > lastHealthPickup + 60000) {
		if (distance(cam.position.x, cam.position.z, healthsphere.position.x, healthsphere.position.z) < 15 && health != 100) {
			health = Math.min(health + 50, 100);
			$('#health').html(health);
			lastHealthPickup = Date.now();
		}
		healthsphere.material.wireframe = false;
	}
	else {
		healthsphere.material.wireframe = true;
	}
...
function setupScene() {
	...
	var floor = new t.Mesh(
			...
			new t.MeshLambertMaterial({map: t.ImageUtils.loadTexture('images/mud.jpg')}));
	scene.add(floor);

	// Geometry: walls
	var cube = new t.CubeGeometry(UNITSIZE, WALLHEIGHT, UNITSIZE);
	var materials = [
	                 new t.MeshLambertMaterial({/*color: 0x00CCAA,*/map: t.ImageUtils.loadTexture('images/brick.jpg')}),
	                 new t.MeshLambertMaterial({/*color: 0xC5EDA0,*/map: t.ImageUtils.loadTexture('images/wallbullet.jpg')}),
...
function addAI() {
	...
	var aiMaterial = new t.MeshBasicMaterial({/*color: 0xEE3333,*/map: t.ImageUtils.loadTexture('images/ussr.png')});
