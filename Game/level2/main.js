...
function setupMap() {
	...
	var material = new THREE.MeshPhongMaterial({ map: THREE.ImageUtils.loadTexture('mud.jpg') });
...
var addVoxel = (function() {
	var XOFFSET = map.length * 0.5 * HORIZONTAL_UNIT,
			...
			materials = [];
	for (var i = 0; i < 8; i++) {
		materials.push(new THREE.MeshPhongMaterial({map: THREE.ImageUtils.loadTexture('brick.jpg')}));
	}
	function material() {
...
function setupEnemies() {
	THREE.ImageUtils.loadTexture('ussr.png', undefined, function(texture) {
